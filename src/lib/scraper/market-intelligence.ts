...
async function collectAdjudicationUrls(
  ...
  const feeds: string[] = [
    ...articleCodes.map((code) => `${RSS_BASE}/articulo/${encodeURIComponent(code)}`),
    ...keywords.map((kw) => `${RSS_BASE}/texto/${encodeURIComponent(kw)}`),
  ]
  ...
  for (const feed of feeds) {
    ...
    if (tenderIds.length >= MAX_DETAILS) break
  }

  return tenderIds.slice(0, MAX_DETAILS).map((id) => `${DETAIL_BASE}/${id}`)
}

function buildCompetitorMap(adjudications: ParsedAdjudication[]): CompetitorProfile[] {
...
export async function analyzeMarket(
  ...
  const detailUrls = await collectAdjudicationUrls(articleCodes, keywords)
  const parsed: ParsedAdjudication[] = []

  for (const url of detailUrls) {
    await delay(300)
    const html = await fetchArceHtml(url)
    if (!html) continue
    ...
    parsed.push({
      ...
      record: {
        tenderId,
        organismo: detail.adjudicatedItems.length > 0 ? organismoFromTitle(opportunity.title) : '',
  ...
  // Steps 3 & 4 — competitor map and price intelligence.
  const competitors = buildCompetitorMap(parsed)
